...
| #16 | 음성 통역            | 🔶   | B안 직결(`geminiLive.ts`). 2026-06-30 서버 `GEMINI_API_KEY` 설정·검증(`gemini-live-token`→ephemeral 발급 정상) + 릴리스는 서버 토큰 경로 강제(DEV_KEY를 `__DEV__` 게이트). 남은 의존: 네이티브 16kHz PCM 마이크 캡처(prebuild) |
| #17 | K-Map 렌더링         | ✅   |                                                                                                                                                                                                                                |
...
> 미검증(외부/대면 필요) — #8 소셜·#9 OTP. #16 잔여는 네이티브 PCM 마이크(prebuild)뿐.
