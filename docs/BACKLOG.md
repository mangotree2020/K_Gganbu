...
| #8  | 소셜 로그인          | 🔶   | **Google ✅ 검증**(2026-06-30 실기기 — provider:google, 실계정 비익명 세션). Apple: 앱 코드·딥링크 준비완료, **Apple Developer 조직가입 D-U-N-S 발급 대기(~2026-07-07)** → 활성화 후 Supabase Apple provider 설정+검증만 남음(안드로이드 웹OAuth로 테스트 가능). **LINE**(Phase2 §6): 커스텀 통합 구현완료 — `line-auth`+`line-callback`(https 중계, LINE 커스텀스킴 불허 대응) Edge Function 배포+`useLINESignIn`. 채널 Channel ID/Secret·시크릿 설정됨, Callback URL=line-callback https 등록 후 실검증 대기(2026-07-01 인증페이지 도달 확인) |
...
> 미설정 — #8 Apple provider(D-U-N-S 대기)·LINE 채널, #9 Phone OTP(Twilio 예정). 코드/Edge Function은 준비 완료, 외부 자격증명만 대기.
