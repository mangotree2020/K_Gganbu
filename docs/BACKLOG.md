...
| #8  | 소셜 로그인          | 🔶   | **Google ✅ 검증**(2026-06-30 실기기 — provider:google, 실계정 비익명 세션). Apple: 앱 코드·딥링크 준비완료, **Apple Developer 조직가입 D-U-N-S 발급 대기(~2026-07-07)** → 활성화 후 Supabase Apple provider 설정+검증만 남음(안드로이드 웹OAuth로 테스트 가능). **LINE**(Phase2 §6): 커스텀 통합 구현완료 — `line-auth` Edge Function 배포(verify_jwt=false)+`useLINESignIn`, LINE Login 채널(Channel ID/Secret) 설정 시 동작 |
